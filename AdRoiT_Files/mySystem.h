...
SetGlobalInterrupt;             // mengaktifkan interrupt dari event yang lain
    ...
    if (myFlag.ImuStart)                            // program ini dijalankan hanya ketika IMU diaktifkan
    {
                ...
                if(X==JumlahFiFo)
                {   imu.getFIFOBytes(imu.FifoBuffer,JumlahFiFo);
                    imu.dmpGetQuaternion16Bit();
                    imu.dmpGetGyroData();
                    LED1(0);
                }
                else
...
{   int16_t x;
 ...
 //-------------- Inisialisasi Timer -----------------------------------
    ...
    TCNT1H=0x00;
    TCNT1L=0x00;
    ICR1 = 5000;    // interrupt pada sisi atas sinyal segi tiga = 2000000 / (2*5500) = 200KHz = 5ms
